...
PROPER_NOUN_GENDER = {
    ...
    # Eldoria characters
    'leo': 'MASC', 'mia': 'FEM', 'sam': 'MASC',
}

# Words that are grammatically singular but semantically refer to a group,
# so "they/them" should be able to bind to them.
COLLECTIVE_WORDS = {
    'everyone', 'everybody', 'someone', 'somebody',
    'anyone', 'anybody', 'no one', 'nobody',
    'all', 'both', 'each', 'group', 'team', 'family',
    'crew', 'band', 'gang', 'party', 'crowd', 'audience',
}
...
FEATURE_WEIGHTS = {
    ...
    'known_target':   1.2,   # soft bonus if candidate matches a caller-supplied
                              # target name (e.g. a known suspect). Deliberately
                              # modest relative to 'parallelism'/'semantic' — this
                              # breaks ties, it does not override strong evidence
                              # that a pronoun refers to someone else. Competing
                              # candidates are never removed from the pool; see
                              # CorefResolver.set_target_names().
    'animacy':        2.5,   # personal pronouns strongly prefer human/animate
                              # candidates over inanimate nouns (fixes "they"
                              # binding to plural objects like "sounds").
}

COREF_THRESHOLD = 0.35   # was 0.55 — too conservative for short narratives
                          # with several competing candidates.
...
class CorefResolver:
    ...
    def __init__(self, encoder):
        ...
        self.candidates = []
        self._eid_to_idx = {}
        # Optional soft-anchoring targets (e.g. a known suspect's name). Stored
        # as lowercased strings; matched against a candidate's canonical text.
        # Empty by default — anchoring is opt-in, never on unless explicitly set.
        self._target_names_lower = set()

    def set_target_names(self, names: List[str]):
        """
        Register one or more names (e.g. a known suspect) to softly bias coref
        resolution toward, via the 'known_target' feature. This does NOT remove
        or hide any other entity from the candidate pool — competing candidates
        remain fully eligible and can still win if the other features (semantic
        similarity, parallelism, recency, etc.) favor them. This only breaks
        ties/near-ties toward a name you already know is relevant, rather than
        masking other entities out of the narrative (which would artificially
        force every remaining pronoun to resolve to the target by removing its
        competition — see design discussion: that approach was rejected because
        it manufactures false-confidence attributions rather than resolving
        genuine ambiguity).
        """
        self._target_names_lower = {n.lower().strip() for n in names if n.strip()}
    ...
    def infer_entity_attrs(self, token):
        ...
        if morph_num:
            ...
            # Proper nouns default to singular even if they happen to end
            # in "s" (e.g. "Sam", "Chris") — the old suffix heuristic was
            # only meant for common nouns.
            number = 'SING'
    ...
    def _f_known_target(self, cand):
        """1.0 if this candidate's canonical text matches a registered target
        name (set via set_target_names), else 0.0. A soft tie-breaker, not a
        filter — see set_target_names() docstring for why."""
        if not self._target_names_lower:
            return 0.0
        return 1.0 if cand['text'].lower().strip() in self._target_names_lower else 0.0

    def _f_animacy(self, cand):
        """Boost human/animate candidates so 'they' doesn't resolve to
        an inanimate plural noun like 'sounds' or 'trees'."""
        if cand['last_token'] is None:
            return 0.0
        if cand['last_token'].pos_ == 'PROPN':
            return 1.0
        person_nouns = {
            'person', 'people', 'man', 'woman', 'child', 'friend', 'companion',
            'hero', 'adventurer', 'elder', 'village', 'everyone', 'everybody',
            'someone', 'somebody', 'anyone', 'anybody', 'nobody',
        }
        if cand['last_token'].text.lower() in person_nouns:
            return 0.9
        return 0.0
    ...
    def resolve(self, pronoun_text: str, pronoun_dep: str,
        ...
        for cand in self.candidates:
            ...
            cand_number = cand['number']
            # Collective words ("everyone", "the group", ...) are
            # semantically plural even though they're grammatically singular.
            if cand['text'].lower() in COLLECTIVE_WORDS:
                cand_number = 'PLUR'

            # HARD FILTER: gender mismatch only.
            if (p_gender != 'UNKNOWN' and cand_gender != 'UNKNOWN'
                    and p_gender != cand_gender):
                log_scores.append((cand['eid'], float('-inf')))
                continue

            # SOFT PENALTY: number mismatch (was a hard filter). This lets
            # "they" still bind to a singular collective like "Everyone"
            # when nothing better is available, instead of being forced
            # onto whatever inanimate plural noun happens to be nearby.
            number_penalty = 0.0
            if (p_number != 'UNKNOWN' and cand_number != 'UNKNOWN'
                    and p_number != cand_number):
                number_penalty = -3.0
            ...
            f = {
                ...
                'known_target':  self._f_known_target(cand),
                'animacy':       self._f_animacy(cand),
            }
            log_score = sum(FEATURE_WEIGHTS[k] * f[k] for k in FEATURE_WEIGHTS)
            log_score += number_penalty
...
class EntityRegistry:
    ...
    def set_target_names(self, names: List[str]):
        """Pass-through to the underlying CorefResolver — see its docstring."""
        self.coref.set_target_names(names)
    ...
    def resolve_or_create(self, text: str, sent_idx: int, is_pronoun: bool,
        ...
        if is_pronoun:
            ...
            if best_eid is not None:
                mention_idx = self._add_mention(best_eid, text, sent_idx,
                                                is_pronoun=True)
                # Refresh the resolver's state for this candidate so later
                # pronouns in the same chain still see it as recent/salient,
                # instead of it silently decaying away (e.g. "Everyone").
                if best_eid in self.coref._eid_to_idx:
                    cand = self.coref.candidates[self.coref._eid_to_idx[best_eid]]
                    cand['last_seen_sent'] = sent_idx
                    cand['mention_count'] += 1
                    cand['last_dep'] = dep or ''
                    if dep in ('nsubj', 'nsubjpass'):
                        cand['subject_count'] += 1
                    # Deliberately keep the original last_token/embedding —
                    # a resolved pronoun shouldn't overwrite the semantic
                    # anchor that got it resolved in the first place.
        ...
        # Register as a candidate for future pronouns — but only referential
        # expressions (nouns, proper nouns, pronouns). Adverbs and other
        # non-referential tokens were previously polluting the candidate
        # pool (e.g. "accidentally" winning as the antecedent for "they").
        if token is not None and token.pos_ in {'NOUN', 'PROPN', 'PRON'}:
            # Skip non-referential locative pronouns ("there"/"here").
            if token.pos_ == 'PRON' and token.text.lower() in {'there', 'here'}:
                pass
            else:
...
def preprocess(corpus: List[str], model_name='all-MiniLM-L6-v2',
    ...
    if target_names:
        registry.set_target_names(target_names)
...
# --------------------------------------------------------------------
# 5b. POST-RESOLUTION TARGET FILTER
# --------------------------------------------------------------------
def filter_to_target_subgraph(output: Dict[str, Any], target_names: List[str]) -> Dict[str, Any]:
    """
    Filters an already-fully-resolved event graph down to events where a named
    target (e.g. a known suspect) participates in some role.

    This is deliberately a POST-processing step, not a preprocessing one. It
    runs after coreference has resolved with the full competing entity pool
    intact, so ambiguous pronouns still had genuine competitors to be judged
    against. Filtering the finished graph down to target-relevant events is a
    safe convenience for downstream Belief/Desire coding; masking entities
    before resolution would not be (it collapses genuine ambiguity into false
    certainty by deleting a pronoun's competition rather than resolving it).

    Does not mutate the input. Entities not referenced by any surviving event
    are dropped from the returned 'entities' dict; everything else (event_types,
    unresolved, feature_log) is passed through unchanged for traceability.
    """
    target_names_lower = {n.lower().strip() for n in target_names if n.strip()}
    if not target_names_lower:
        return output

    def entity_matches_target(entity_id: Optional[int]) -> bool:
        if entity_id is None:
            return False
        canonical = output['entities'].get(entity_id, {}).get('canonical_text', '')
        return canonical.lower().strip() in target_names_lower

    kept_events = []
    kept_entity_ids = set()
    for ev in output['events']:
        if any(entity_matches_target(r['entity_id']) for r in ev['roles']):
            kept_events.append(ev)
            for r in ev['roles']:
                if r['entity_id'] is not None:
                    kept_entity_ids.add(r['entity_id'])

    kept_entities = {eid: ent for eid, ent in output['entities'].items()
    ...
    return {
        ...
        'feature_log': output['feature_log'],
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--target-names', type=str, default=None,
                         help='Comma-separated name(s) of a known target/suspect '
                              '(e.g. "Kevin,Kevin Jong"). Softly biases coreference '
                              'toward these names on ties, without removing other '
                              'entities from consideration.')
    parser.add_argument('--filter-to-target', action='store_true',
                         help='After full resolution, drop events/entities that '
                              'do not involve --target-names. Has no effect if '
                              '--target-names is not also given.')
    ...
    if args.filter_to_target and args.target_names:
        output = filter_to_target_subgraph(output, args.target_names.split(','))
        print(f"Filtered to target subgraph: {len(output['entities'])} entities, "
              f"{len(output['events'])} events remain.")
